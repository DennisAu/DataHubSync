...
Write-Host "=========================================="
Write-Host "DataHubSync Hub Startup (PowerShell - Hub Local)"
Write-Host "=========================================="
...
"@
    ...
    try {
        ...
        $errors += "Failed to execute config parsing script: $_"
    } finally {
...
if ($errors.Count -eq 0) {
    ...
    if (-not $cacheDir) {
        ...
        Write-Warn "cache_dir directory not found: $cacheDir - attempting to创建..."
        try {
...
try {
    # Use the parsed data_root and cache_dir values
    $dataRoot = $cfg | Select-Object -ExpandProperty data_root -ErrorAction SilentlyContinue
    $cacheDir = $cfg | Select-Object -ExpandProperty cache_dir -ErrorAction SilentlyContinue
    
    Write-Host "Starting server with:"
    Write-Host "  Config: $ConfigPath"
    Write-Host "  Server: $ServerPath"
